...
def DerivativeSignal():
    ...
    def first_derivative(x):
        ...
        result = [x[i] - x[i - 1] for i in range(1, len(x))]
        return result

    def second_derivative(x):

        result = [x[i + 1] - 2 * x[i] + x[i - 1] for i in range(1, len(x) - 1)]

        return result

    FirstDrev =first_derivative(InputSignal)
    SecondDrev =second_derivative(InputSignal)
    print(len(FirstDrev), "    ", len(SecondDrev), "     ", len(expectedOutput_first), "     ",
          len(expectedOutput_second))
